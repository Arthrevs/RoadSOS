...
\author{
  \small\color{rsgrey} \texttt{github.com/Arthrevs/RoadSOS} \\
...
|                                                               |
|   React 18  +  Vite 7.3.3  +  Workbox 7 (Service Worker)          |
|   |- Leaflet 1.9.4    (real OSM map, dark tiles)              |
|   |- i18next 26       (48 languages, RTL support)             |
|   |- localStorage     (24h search-result cache)               |
|   `- bundled JSON     (938 entries x 200 countries)          |
+----------------------------+----------------------------------+
...
|   |- /triage    (Gemini 2.5 Flash + rule-based fallback)      |
...
Build tool & Vite 7.3.3, vite-plugin-pwa 1.3.0 (\code{injectManifest}) \\
...
i18n & i18next 26.2.0 + react-i18next 17.0.8; 48 locales \\
Icons & lucide-react 1.16.0 \\
Backend runtime & FastAPI 0.115.0 + uvicorn 0.30.6, Python 3.11.9 \\
Async HTTP & httpx 0.27.2 \\
AI & Google Gemini 2.5 Flash via REST (no SDK; uses httpx) \\
...
The \code{GOOGLE\_PLACES\_API\_KEY} environment variable is parsed as a comma-separated list of API keys. Round-robin rotation distributes load across multiple billing accounts:

\begin{lstlisting}[language=Python,caption={googleplaces\_service.py:30--36}]
def _load_keys() -> list[str]:
    multi = os.getenv("GOOGLE_PLACES_API_KEY", "")
...
\begin{lstlisting}[caption={Sample /search response}]
{
  "contacts": [
    {
      "id": "node/8127364521",
...
\begin{lstlisting}[numbers=none,caption={Response when all upstreams fail}]
{
  "contacts": [],
...
\begin{lstlisting}[caption={offlineDB.js: 24-hour client-side cache}]
...
When network requests fail and no cache entry exists, \file{src/utils/bundledFacilities.js} computes the nearest entries from a JSON file shipped in the build bundle: \file{src/data/bundled\_facilities.json} (938 entries, 200 countries).
...
\begin{lstlisting}[caption={i18n/index.js --- applyDir}]
...
On first launch, the user sees a full-screen modal listing all 48 languages, grouped into ``India'' (22) and ``World'' (26) sections. There is no automatic pre-selection based on GPS country code; the user picks explicitly. The choice is persisted to \code{localStorage['roadsos:lang']} and never re-asked.
...
The hook maintains a rolling buffer of GPS samples (each containing speed in m/s and a timestamp). A velocity collapse is registered when sustained samples at $\ge$ 40 km/h are followed within ~2.5 seconds by a sample at $\le$ 5 km/h (tuned to reject ordinary braking and stop-and-go traffic).
...
\begin{lstlisting}[numbers=none,caption={Request body}]
{
...
\begin{enumerate}[leftmargin=*, itemsep=2pt, topsep=2pt]
  \item \textbf{lint.} \code{ruff check} (rule set: E, W, F, I, UP, B, C4, SIM) followed by \code{ruff format --check}. Four rules are explicitly ignored: E501 (line length), E402 (intentional \code{load\_dotenv()} ordering), B008 (FastAPI \code{Depends()}), and SIM108 (ternary readability).
  ...
  - type: web
    ...
    envVars:
      - key: GEMINI_API_KEY        # required, free at aistudio.google.com
        sync: false
      - key: GOOGLE_PLACES_API_KEY       # optional, comma-separated for multi-key
        sync: false
      - key: PYTHON_VERSION
        value: 3.11.9
\end{lstlisting}
...
\begin{itemize}[leftmargin=*, itemsep=2pt, topsep=2pt]
  \item \code{VITE\_API\_URL} --- e.g.\ \code{https://roadsos-pl3k.onrender.com}
\end{itemize}
...
AI model & \code{gemini-2.5-flash} (free tier: 15 RPM / 1500 RPD) \\
