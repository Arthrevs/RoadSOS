...
Number of emergency contacts found &
  ...
  Auto-expanding radius 8\,km $\to$ 25\,km. \\
Reliability &
  Every external call wrapped in \code{\_safe\_*} helpers. 3 Overpass mirrors raced concurrently (first healthy response wins). API always returns HTTP 200 with a valid shape. \\
Offline functionality &
  Four-tier fallback: live backend $\to$ 7-day localStorage cache $\to$ bundled JSON
  (938 entries, 200 countries) $\to$ national emergency-numbers banner. Country emergency numbers
  always offline. \\
...
\begin{itemize}
  \item Google Gemini 2.5 Flash is used for situational triage via direct REST (no SDK). Free-tier limits (60 RPM / 1500 RPD) are sufficient for a demo.
...
\midrule
OpenStreetMap Overpass API & Public OSM data query. Three mirrors raced concurrently (first healthy response wins). \\
...
Google Gemini 2.5 Flash & AI triage. Free tier: 60\,RPM / 1\,500\,RPD. \\
...
\begin{enumerate}
  ...
  \item Phase 1: parallel fan-out --- Nominatim reverse geocode + Overpass query
        (9 OSM categories, 3 mirrors, 10\,s per-mirror timeout).
  ...
  \item Frontend: render on Leaflet map (CartoDB Dark Matter tiles), populate
        Quick-Contacts dock, persist to 7-day localStorage cache.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{Tier 3} --- Bundled JSON shipped with the PWA:
        938 entries across 200 countries.
